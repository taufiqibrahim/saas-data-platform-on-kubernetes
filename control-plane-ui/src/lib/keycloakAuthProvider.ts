...
export const keycloakAuthProvider = (
  ...
  async logout() {
    ...
    await localStorage.removeItem("realm")
  ...
  async checkAuth() {
    // 1. Check if the realm is already in localStorage
    const realm = localStorage.getItem('realm');

    // If no realm, redirect to custom realm selection page
    if (!realm) {
      window.location.href = '/realm';
      throw new Error('Realm not selected');
    }

    // 2. If realm exists, proceed with standard Keycloak check
    await initKeyCloakClient(keycloakClient, options.initOptions);
    await isAuthenticated(keycloakClient, options.authenticationTimeout);
    if (keycloakClient.authenticated && keycloakClient.token) {
      return;
...
const initKeyCloakClient = async (
  ...
  // If the client isn't configured for the selected realm yet, update it
  if (storedRealm && keycloakClient.realm !== storedRealm) {
    // Most Keycloak JS clients allow updating the realm before init
    (keycloakClient as any).realm = storedRealm;
  }
  ...
  // Only init if we actually have a realm to target
  if (storedRealm) {
